...
const App: React.FC = () => {
  ...
  const handleAdminDelete = async (id: string) => {
    ...
    try {
      ...
      // Opcional: Se der erro no banco, poderíamos avisar o usuário que a exclusão persistente falhou
    }
  };
    ...
    <div className="p-4 pb-32 space-y-2.5 animate-in fade-in duration-500">
      <header className="flex justify-between items-start">
        <div>
          ...
          <button onClick={() => setCurrentView('TOKENIZE')} className="mt-2.5 h-7 px-4 bg-amber-500 text-slate-950 rounded-full text-[8px] font-black uppercase tracking-[0.15em] shadow-lg shadow-amber-500/20 active:scale-90 transition-all border border-amber-400/40 flex items-center gap-1.5">
        ...
        <div className="flex flex-col items-center gap-1.5">
          <div 
            ...
            className="h-16 w-16 bg-slate-800 rounded-full flex items-center justify-center border-[2px] border-yellow-400 shadow-xl transition-all overflow-hidden relative cursor-pointer active:scale-95 group"
          >
            {userProfile.avatarUrl ? (
              ...
              <i className="fa-solid fa-user text-2xl text-yellow-400"></i>
            )}
          </div>
          <span className="text-yellow-400 text-[9px] font-black uppercase tracking-widest leading-none text-center max-w-[70px]">
             {displayName}
          </span>
        </div>
      </header>
      ...
      <section className="space-y-2">
        ...
        <div className="space-y-2">
           ...
           <div className="flex gap-3 overflow-x-auto pb-1.5 -mx-4 px-4 scrollbar-hide snap-x">
              ...
              {assets.map((asset) => (
                 <div key={asset.id} className="relative shrink-0 snap-start">
                    <button onClick={() => handleAdminEdit(asset)} className={`min-w-[140px] h-14 rounded-2xl border flex items-center gap-3 px-3 transition-all relative group overflow-hidden ${String(editorData.id) === String(asset.id) ? 'bg-white border-white text-slate-950 shadow-lg' : 'bg-slate-900/50 border-slate-800 text-slate-500'}`}>
                        ...
                        <span className="text-[10px] font-black uppercase tracking-tighter truncate leading-tight pr-6">{asset.title}</span>
                    </button>
                    {/* Botão de Exclusão Rápida para eficácia imediata */}
                    <button 
                      onClick={(e) => { e.stopPropagation(); handleAdminDelete(asset.id); }} 
                      className="absolute top-1/2 -translate-y-1/2 right-2 h-7 w-7 bg-red-500/10 hover:bg-red-500 text-red-500 hover:text-white rounded-lg flex items-center justify-center transition-all active:scale-75 z-20 border border-red-500/20"
                    >
        ...
        <div className="fixed inset-0 z-[100] flex items-end sm:items-center justify-center p-0 sm:p-6 animate-in fade-in duration-300">
           ...
           <div className="bg-slate-900 border-t sm:border border-slate-800 p-8 rounded-t-[2.5rem] sm:rounded-[2.5rem] w-full max-md:max-w-md relative z-10 shadow-2xl space-y-6 animate-in slide-in-from-bottom duration-300">
                ...
                <div className="pt-2 gap-3 flex flex-col">
                    <button onClick={handlePurchase} disabled={isLoading || userBalance < totalCost} className="w-full bg-amber-500 disabled:bg-slate-800 disabled:text-slate-500 text-slate-950 font-black py-4 rounded-xl text-[11px] uppercase tracking-[0.2em] shadow-lg active:scale-98 transition-all flex items-center justify-center gap-2">{isLoading ? <i className="fa-solid fa-circle-notch fa-spin"></i> : <i className="fa-solid fa-check"></i>}{isLoading ? 'Processando...' : 'Confirmar Compra'}</button>
      ...
      <div className="p-0 pb-32 animate-in slide-in-from-right duration-500 bg-slate-950 min-h-screen">
        ...
        <div className="pt-20 flex flex-col">
            {allGalleryItems.map((item, index) => {
                const itemPrice = (item as GalleryItem).fractionPrice !== undefined ? (item as GalleryItem).fractionPrice : (index === 0 ? selectedAsset.fractionPrice : selectedAsset.fractionPrice * (1 + (index * 0.15)));
                ...
                <div key={item.id} className="mb-12 last:mb-0 animate-in fade-in duration-700">
                   ...
                   <div className="px-6 mt-1 space-y-0">
                      ...
                      <div className="bg-slate-900/60 border border-slate-800 p-2.5 rounded-xl shadow-xl relative overflow-hidden backdrop-blur-md mb-4">
                          ...
                          <div className="space-y-3 mt-2">
                            <div className="flex items-center justify-between gap-3 bg-slate-950/40 p-1 rounded-xl border border-slate-800/30">
                               ...
                               <div className="bg-amber-500 text-slate-950 p-2.5 rounded-lg flex flex-col items-center justify-center min-w-[85px]">
                      ...
                      <div className="bg-slate-900/40 border border-slate-800/80 p-2.5 rounded-xl space-y-3 shadow-2xl mb-8">
                         <div className="grid grid-cols-2 gap-3">
                            <div className="space-y-0.5">
                               <p className="text-slate-500 text-[8px] font-black uppercase tracking-widest opacity-60">Valor da Obra</p>
                            ...
                            <div className="space-y-0.5 text-right">
                               <p className="text-amber-500 text-[8px] font-black uppercase tracking-widest opacity-60">Preço / Fração</p>
                               <div className="flex items-baseline gap-1 justify-end">
                                  <span className="text-amber-500 font-bold text-[10px]">R$</span>
                                  <span className="text-amber-500 text-lg font-black tracking-tighter">
                         ...
                         <div className="pt-3 border-t border-slate-800/50">
                            <button 
                              onClick={() => setPurchaseAsset({...selectedAsset, ...item, fractionPrice: itemPrice, quantity: 1})} 
                              className="w-full bg-amber-500 text-slate-950 font-black py-3.5 rounded-xl text-xs uppercase tracking-[0.4em] shadow-lg active:scale-98 flex items-center justify-center gap-2"
                            >
                               <i className="fa-solid fa-cart-plus"></i> COMPRAR FRAÇÃO
                            </button>
                         </div>
                      </div>
                   </div>
  ...
  return (
    <div className="max-w-md mx-auto min-h-screen bg-slate-950 relative shadow-2xl overflow-x-hidden ring-1 ring-slate-800 antialiased selection:bg-amber-500/40">
      ...
      {renderPurchaseModal()}
      {!['ADMIN', 'ADMIN_LOGIN', 'CUSTODY_GALLERY'].includes(currentView) && (
